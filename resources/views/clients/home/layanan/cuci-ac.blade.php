...
@section('title', 'Cuci AC')
@section('content')
    <div class="container p-5">
        <div class="row">
            <div class="col-lg-6">
                <h1 class="fw-bold text-uppercase">Cuci AC</h1>
                <form method="post" action="{{ route('buat_transaksi') }}">
                    @csrf
                    <div class="mb-3">
                        <div class="form-label">Jenis Jasa</div>
                        <input type="text" class="form-control" name="layanan" value="Cuci Ac" readonly>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" name="biaya_jasa" id="biaya_jasa" value=""
                            readonly>
                    </div>
...
@push('script')
    <script>
        $('input[type=radio][name=type_rumah]').change(function() {
            if (this.value == 'Rumah') {
                $('input[type=number][name=biaya_jasa]').val(60000);
            } else if (this.value == 'Ruko') {
                $('input[type=number][name=biaya_jasa]').val(70000);
            } else if (this.value == 'Apartement') {
                $('input[type=number][name=biaya_jasa]').val(100000);
            }
        });
